...
const generateImageWithAIAndMakeStory = async (req, res) => {
  ...
  // Description ko URL encoded format me convert karein
  const encodedPrompt = encodeURIComponent(description);
  
  // Pollinations AI API URL (Free)
  const imageUrl = `https://pollinations.ai/p/${encodedPrompt}?width=1024&height=1024&seed=42`;
    ...
    const response = await axios.post(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${process.env.GEMINI_API_KEY}`,
      {
        contents: [
          {
            role: "user",
            parts: [{ text: "Give 100 fruit names" }],
          },
        ],
      }
    );
...
module.exports = { generateQuestionPaper ,getFruits,getAnimals,generateImageWithAIAndMakeStory};